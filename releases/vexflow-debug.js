/**
 * VexFlow 1.2.27 built on 2015-05-13.
    ...
    if (node.nodeName == "part-list") this.parsePartList(node);
      ...
      for (var j = 0; j < node.childNodes.length; j++) {
        ...
        if (measure.nodeName != "measure") continue;
                               ...
                               if(attr) last_attr = attr;
                               }
                               
                               if (! (measureNum in this.measures)){
                                                              if (options) {
                               if (measure_index<(parseInt(options.length) + parseInt(options.start)) && measure_index >= parseInt(options.start)) {
//                               document.write("<br> add" + measure_index);
                               measure_index++;
        ...
        this.measures[measureNum][partNum] = measure;
                               if(!last_attr || !options){
                               var attributes = measure.getElementsByTagName("attributes")[0];
                               if(attributes) last_attr = attributes;
                               }

        if (last_attr) this.parseAttributes(measureNum, partNum, last_attr);
...
Vex.Flow.Backend.MusicXML.prototype.getMeasure = function(m) {
  ...
  var time = measure_attrs.time;
    ...
    measure.divisions = measure_attrs.divisions;
          ...
          for (var i = 0; i < directionElems.length; i++) {
              ...
              if (directionTypeElems[0]) {
                  for (var j = directionTypeElems.length - 1; j >= 0; j--) {
                      ...
                      if (metronome) {
//                          document.write(directionTypeElems.length);
                          ...
                          }[metronome.getElementsByTagName("beat-unit")[0].textContent];
                          //速度值
                          per_minute = parseInt(metronome.getElementsByTagName("per-minute")[0].textContent);
                          measure_tempo = Math.floor(per_minute * 4/beat_unit);
      ...
      measure.beat_unit = beat_unit;
      measure.per_minute = per_minute;
      measure.measure_tempo = measure_tempo;
      this.measures[m].beat_unit = beat_unit;
      this.measures[m].per_minute = per_minute;
      this.measures[m].measure_tempo = measure_tempo;
...
Vex.Flow.Backend.MusicXML.prototype.get_previoussibling = function(n){
...
//                                                  attrs.divisions).simplify();
                               var intrinsicTicks = parseInt(elem.textContent);
        if (isNaN(intrinsicTicks))
          ...
          switch (notationElem.nodeName) {
            case "tied": // start-start/stop-stop vs begin-continue-end
              ...
              break;
                                     ...
                                     switch (slu) {
                                     case "start":
                                     noteObj.slur = (noteObj.slur == "end") ? "continue" : "begin";
                                     break;
                                     case "stop":
                                     noteObj.slur = (noteObj.slur == "begin") ? "continue" : "end";
                                     break;
                                     default: Vex.RERR("BadMusicXML", "Bad tie: " + tie.toString());
                                     ...
                                     if (staccato) {
                                     var positionY = (noteObj.stem_direction==1||noteObj.beam)?4:3;//3:above 4:below
//                                     var position = staccato.getElementsByTagName["placement"][0];
//                                     if (position) {
//                                     positionY = {"above":3,"below":4}[position.textContent];
//                                     };
                                     noteObj.staccato = positionY;
                                     };
                                     break;
  ...
  if (noteObj.rest && ! noteObj.keys) {
    ...
    if (noteObj.fingering==0 && !noteObj.rest) {
        var direction = this.get_previoussibling(noteElem);
        while (direction && direction.nodeName == "direction" && direction.getElementsByTagName("direction-type")[0]) {
            var directionType = direction.getElementsByTagName("direction-type")[0];
            direction = this.get_previoussibling(direction);
            Array.prototype.forEach.call(directionType.childNodes, function(directionElem){
                                         switch (directionElem.nodeName){
                                         case "words":
                                            noteObj.fingering = parseInt(directionElem.textContent);
                                         break;
                                         case "wedge"://渐强{crescendo:diminuendo:stop}
                                         var wedge = directionElem.getAttribute("type");
                                         break;
                                         case "dynamics"://力度
                                            Array.prototype.forEach.call(directionElem.childNodes, function(dynamic){
                                                                         var velocity = dynamic.nodeName;
                                                                         });
                                         break;
                                         }
                                         });
//            var words = directionType.getElementsByTagName("words")[0];
//            if (words) {
 ...
 * @constructor
 */
Vex.Flow.Document = function(data, options, measureoptions) {
  if (arguments.length > 0) this.init(data, options, measureoptions);
}

Vex.Flow.Document.backends = [Vex.Flow.Backend.IR, Vex.Flow.Backend.MusicXML];

Vex.Flow.Document.prototype.init = function(data, options, measureoptions) {
  this.options = {};
  Vex.Merge(this.options, options);
  this.measures = new Array();
  if (! data) {
  ...
  for (var i = 0; i < backends.length; i++) {
    ...
    if (Backend.appearsValid(data)) {
      ...
      this.backend.parse(data, measureoptions);
...
var sluredNote = null;
Vex.Flow.DocumentFormatter.prototype.getVexflowVoice =function(voice, staves, options){
...
//          vexflowObjects.push(new Vex.Flow.Curve(tiedNote, vfNote, {
//                    cps: [{x: 0, y: 15}, {x: 0, y: 15}]
//                    }));
          // TODO: Tie only the correct indices
          vexflowObjects.push(new Vex.Flow.StaveTie(
                                                    {first_note: tiedNote, last_note: vfNote, first_indices: [0], last_indices: [0]
                                                    }));
          tiedNote = null;

      }else if(options && i == voice.notes.length - 1 && tiedNote != null && options.system_end){//是一行末
//          vexflowObjects.push(new Vex.Flow.StaveTie(
//                                                    {first_note: tiedNote, last_note: null
//                                                    }));
//          vexflowObjects.push(new Vex.Flow.Curve(tiedNote, null, {
//                                                 cps: [{x: 0, y: 15}, {x: 0, y: 15}]
//                                                 }));
    ...
    if (note.tie == "begin" || note.tie == "continue") tiedNote = vfNote;
      if (note.slur == "end" || note.slur == "continue"){
//          vexflowObjects.push(new Vex.Flow.TextNote({glyph: "p", duration: "16"}));
          var curve = new Vex.Flow.Curve(sluredNote, vfNote, {
                                        cps: [{x: 0, y: 10}, {x: 0, y: 10}],
                                         y_shift:8
                                        });
          vexflowObjects.push(curve);
          // TODO: Tie only the correct indices
//                    vexflowObjects.push(new Vex.Flow.StaveTie(
//                                                              {first_note: sluredNote, last_note: vfNote, first_indices: [0], last_indices: [0]
//                                                              }));
          sluredNote = null;
          
      }else if(options && i == voice.notes.length - 1 && sluredNote != null && options.system_end){//是一行末
//                    vexflowObjects.push(new Vex.Flow.StaveTie(
//                                                              {first_note: sluredNote, last_note: null
//                                                              }));
          var curve = new Vex.Flow.Curve(sluredNote, null, {
                                         cps: [{x: 0, y: 10}, {x: 0, y: 10}],
                                         y_shift:8
                                         });
          vexflowObjects.push(curve);
      ...
      if (note.slur == "begin" || note.slur == "continue") sluredNote = vfNote;
...
Vex.Flow.DocumentFormatter.prototype.getVexflowNote = function(note, options) {
  ...
  vfNote.tag = note.tag;
  vfNote.fingering = note.fingering;
    if (note.fingering != 0 && !note.reset) {
        vfNote.addModifier(0, new Vex.Flow.Annotation(note.fingering).setJustification(1).setVerticalJustification(1));
    };
    if(note.staccato > 0){
        vfNote.addArticulation(0, new Vex.Flow.Articulation("a.").setPosition(note.staccato));
    }
//transparent
    var color = 'transparent';
    vfNote.setStyle({shadowBlur:0, shadowColor:color, fillStyle:color, strokeStyle:color});
  return vfNote;
}

Vex.Flow.DocumentFormatter.prototype.blackNote = function() {
    this.colorNote('black');
...
Vex.Flow.DocumentFormatter.prototype.colorNote = function(color) {
...
Vex.Flow.DocumentFormatter.prototype.colorNoteAt = function(key, color) {
    ...
    {
        //blockDimensions
        var b = vfNote.getStave().block;
        var blockBottom = 0;
        var blockTop = 0;
        var extraMargin = 30;
        for(var i = 0;i<=b;i++){
            blockBottom+= Math.ceil(this.blockDimensions[i][1] * this.zoom) * (this.scale>1?1:this.scale);
            if(i<b){
                blockTop+= Math.ceil(this.blockDimensions[i][1] * this.zoom) * (this.scale>1?1:this.scale);
            }
        }
        blockBottom+= Math.ceil(extraMargin * this.zoom) * (this.scale>1?1:this.scale);
        var currentY = document.body.scrollTop + document.body.clientHeight;
//        document.location = "jslog:" + "zoom:" + this.zoom + "scale:" + this.scale;

//        document.location = "jslog:" + "currentY:" + currentY + "blockBottom:" + blockBottom + "zoom:" + this.zoom + "scale:" + this.scale;
        if(currentY < blockBottom){
            window.scrollTo(0,blockBottom - document.body.clientHeight);
        }else if(document.body.scrollTop>blockTop){
    ...
    {
//        document.location = "jslog:" + "err:" + err;
    }
...
Vex.Flow.DocumentFormatter.prototype.getMinMeasureWidth = function(m) {
  if (! (m in this.minMeasureWidths)) {
    ...
    this.minMeasureWidths[m] = Vex.Max(50,
             maxExtraWidth + noteWidth + maxTickables*10 + 10 + 50);
    ...
    var staveH = vfStaves[0].getYForLine(4) - staveY + 40;
...
Vex.Flow.DocumentFormatter.prototype.drawPart =
  function(part, vfStaves, context, b, options) {
  var staves = part.getStaves();
  var voices = part.getVoices();
      vfStaves.forEach(function(stave) {stave.block=b; stave.setContext(context).draw(); });

  var allVfObjects = new Array();
  var vfVoices = new Array();
  voices.forEach(function(voice) {
    var result = this.getVexflowVoice(voice, vfStaves, options);
...
Vex.Flow.DocumentFormatter.prototype.drawMeasure =
  function(measure, vfStaves, context, options, block) {
  var startStave = 0;
  var parts = measure.getParts();
  parts.forEach(function(part) {
    var numStaves = part.getNumberOfStaves();
    var partStaves = vfStaves.slice(startStave, startStave + numStaves);
    this.drawPart(part, partStaves, context, block, options);
    startStave += numStaves;
  }, this);
...
//    s.addModifier({type:"tempo", tempo:{name:"test", duration:"q", dots:1, bpm:80}});
                                                             if(staveIndex++==0){
                                                             var measure_first = doc.getMeasure(startMeasure);
                                                             var bpm = measure_first.per_minute;
                                                             var unit = measure_first.beat_unit;
                                                             var du = {
                                                             "1":"o" , "2": "h", "4": "q", "8": "8", "16": 16,
                                                             "32": 32, "64": 64, "128": 128, "256": 256
                                                             }[unit+""] + "";
                                                             if(bpm>0){
                                                             s.addModifier({type:"tempo",automatic: false, tempo:{duration:du, dots:0, bpm:bpm}});
                                                             }
...
Vex.Flow.DocumentFormatter.Liquid.prototype.draw = function(elem, options) {
  ...
  renderWidth = Math.floor(canvasWidth / this.zoom);
  ...
  while (this.getBlock(b)) {
    ...
    var lineAnchor = document.createElement("div");
